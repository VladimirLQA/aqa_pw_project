 ...
 * @example
 ...
 ```
 */
export const logStep = <This, Args extends any[], Return>(message?: string) => function actualDecorator(target: (this: This, ...args: Args) => Promise<Return>,
  context: ClassMethodDecoratorContext<This, (this: This, ...args: Args) =>
  Promise<Return>>) {
  function replacementMethod(this: any, ...args: Args) {
    const [_selector, value, ...options] = args;
    const isSecretObj = options.find((option) => typeof option === 'object' && 'isSecret' in option);
